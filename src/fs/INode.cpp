...
void Inode::WriteI()
{
	...
	while (u.u_IOParam.m_Count != 0)
	{
		...
		if (u.u_error != User::_NOERROR) /* 写过程中出错 */
		{
			bufMgr.Brelse(pBuf);
		}
...
int Inode::Bmap(int lbn)
{
	...
	if (lbn < 6) /* 如果是小型文件，从基本索引表i_addr[0-5]中获得物理盘块号即可 */
	{
		 ...
		 * 与物理盘块号之间的映射。
		 */
		if (phyBlkno == 0 && (pFirstBuf = fileSys.Alloc(this->i_dev)) != nullptr)
		{
			/*
	...
	else /* lbn >= 6 大型、巨型文件 */
	{
		...
		if (0 == phyBlkno)
		{
			this->i_flag |= Inode::IUPD;
			/* 分配一空闲盘块存放间接索引表 */
			if ((pFirstBuf = fileSys.Alloc(this->i_dev)) == nullptr)
			{
				return 0; /* 分配失败 */
			}
		...
		if (index >= 8) /* ASSERT: 8 <= index <= 9 */
		{
			...
			if (0 == phyBlkno)
			{
				if ((pSecondBuf = fileSys.Alloc(this->i_dev)) == nullptr)
				{
		...
		if ((phyBlkno = iTable[index]) == 0 && (pSecondBuf = fileSys.Alloc(this->i_dev)) != nullptr)
		{
...
void Inode::Plock()
{
	while (this->i_flag & Inode::ILOCK)
	{
		...
		this->i_cv.wait(i_lock, [this]
										{ return !(this->i_flag & Inode::ILOCK); });
	}
	this->i_flag |= Inode::ILOCK;
}
